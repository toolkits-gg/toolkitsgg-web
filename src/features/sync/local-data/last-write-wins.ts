/**
 *  Last-write-wins conflict resolution used when syncing pending ops to the created-builds.
 */
 ...
 * - `created-builds-wins` — the created-builds record is newer; treat as a conflict and surface it.
...
/**
 * Determines the LWW outcome for a pending op against the current created-builds record.
 *
 * `op.serverUpdatedAt` is the snapshot of the created-builds record's `updatedAt` at the time the
 ...
 *    fall back to comparing the op's own creation time to the created-builds record's time.
...
const compareTimestamps = (
	...
	// No created-builds record — local op wins unconditionally.
