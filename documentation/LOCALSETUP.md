...
If you get stuck, ask in the [Discord](https://discord.gg/VQF23tPKyK) - we want to help. If something here is confusing, wrong, or out of date, that's our bug; please [open an issue](https://codeberg.org/toolkitsgg/toolkitsgg-web/issues).
...
```bash
git clone https://codeberg.org/toolkitsgg/toolkitsgg-web.git
cd toolkitsgg-web
```
...
| `SUPER_ADMIN_EMAIL` / `SUPER_ADMIN_PASSWORD` | Credentials for the site's super admin, the account that can grant roles to everyone else | Leave the defaults from `.env.local.example`                          |
| `LOCAL_USER_EMAIL` / `LOCAL_USER_PASSWORD`   | Credentials for the seeded local user account (`pnpm db:seed:reset`)                      | Leave the defaults from `.env.local.example`                          |
...
> [!IMPORTANT]
> The `LOCAL_*` variables are read only by `prisma/seed-reset.ts`, and the optional `LOCAL_MODERATOR_*` pair seeds an account holding a Remnant 2 `MODERATOR` role so you can exercise the admin tools locally. `SUPER_ADMIN_*` is different: it is validated in every environment, and `pnpm db:seed` guarantees that account exists with a global `SUPERADMIN` role. Re-running the seed never rewrites an existing account's password, so changing the value in `.env.local` does not rotate it.
...
| `REDDIT_CLIENT_ID` / `REDDIT_CLIENT_SECRET`   | Lets you sign in with Reddit locally. Create a **web app** at [reddit.com/prefs/apps](https://www.reddit.com/prefs/apps) with the redirect URI `http://localhost:3000/api/auth/callback/reddit`; the client ID is the string under the app name and the secret is the `secret` field. Reddit never shares the account's email, so Reddit sign-ups get an unusable placeholder address until the user sets a real one. |
...
| `SENTRY_AUTH_TOKEN`                           | Only used at build time, to upload source maps to Sentry so production stack traces are readable. Leave it blank locally; error reporting itself works without it.                                                                                              |

> [!IMPORTANT]
> All env vars are validated by Zod (a TypeScript validation library) in `src/env/` (`server-env.ts` and `client-env.ts`). If you start the app with a missing or malformed value, you'll see a clear error message at startup. Read it; it usually tells you exactly what's wrong.
>
...
- **`pnpm db:generate`** generates that TypeScript code: the server client, which talks to Postgres. The browser's IndexedDB stores are hand-rolled in `features/local-db/`, but typed with the model types from the same generated client, so both halves still come from one schema.
...
> [!WARNING]
> `db:seed:reset` deletes every user and all user-generated game data before recreating the test accounts, so only ever point it at your local database. `pnpm db:seed` on its own is the safe version: it upserts reference data and deletes nothing. See [prisma/SEED.md](../prisma/SEED.md).
...
| `pnpm test`          | Run the test suite once (tests not yet implemented, surprise!)  |
...
| `pnpm db:seed`       | Upsert reference data (deletes nothing, safe anywhere)          |
| `pnpm db:seed:reset` | Wipe local users and game data, then recreate the test accounts |
...
```bash
pnpm generate:favicons    # regenerate favicons from per-game source files
pnpm generate:images      # regenerate resized item images
```
